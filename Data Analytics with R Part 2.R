...
#Creating a visualization to illustrate the distribution of values for few selected values of Total Fat, Protein and Carbohydrate.  
boxplot(USDAclean$TotalFat, USDAclean$Protein, USDAclean$Carbohydrate)
dev.copy(jpeg,'distribution.jpeg')
dev.off()
#Creating a visualization to illustrate the relationship between a food's Total Fat content and its calorie content.
plot(USDAclean$TotalFat, USDAclean$Calories)
dev.copy(jpeg,'FatCalories.jpeg')
dev.off()
#Creating a logistic regression model, using High Calories as the dependent variable, and Carbohydrate, Protein, Total Fat and Sodium as independent variables. (18 p) USDALog = glm(HighCals ~ Carbohydrate + Protein + TotalFat + Sodium, data = USDAclean, family=binomial)
# Which independent variable is not significant? 
summary(USDALog)
# Which independent variable has the strongest positive predictive power in the model? (10 p) summary(USDALog)
#Total Fat
